...
(defun nano-mu4e-dashboard-colors ()
  ...
  (face-remap-add-relative 'org-verbatim 'nano-light-salient)
...
(setq mu4e-thread-folding-root-folded-prefix-string
      (concat
       (propertize "" 'face `(:foreground ,nano-light-faded :height 120))
       " "))
  ...
  (let ((map (make-sparse-keymap)))
    ...
    (concat 
     (propertize text
                'display tag
                'face face
                'mouse-face `(:foreground ,nano-light-salient)
                'local-map map
...
(defun mu4e-headers-button (text face help query)
  ...
  (let ((map (make-sparse-keymap)))
    ...
    (propertize text
                'face face
                'mouse-face `(:foreground ,nano-light-background
...
(defun mu4e-headers-relative-date (msg)
  (let* ((thread  (mu4e-message-field msg :thread))
         ...
         (face 'nano-light-salient))

    ;; (setq face (cond ((= diff   0) (if unread 'nano-light-strong 'nano-light-faded))
    ;;                 ((< diff   8) (if unread 'nano-light-strong 'nano-light-faded))
    ;;                 (t            (if unread 'nano-light-strong 'nano-light-faded))))
    ;; (setq face (if unread 'nano-light-strong 'nano-light-faded))
    (setq face 'nano-light-faded)
    ;; (if (and (not empty-parent) child)
    ;;    (setq face `(:foreground ,nano-subtle)))
...
(defun mu4e-headers-sender-subject (msg)
  (let* ((thread  (mu4e-message-field msg :thread))
    ...
    (concat
     (if (and (not empty-parent) child)
     ;; (if (> level 0)
         (propertize "│ " 'face `(:foreground ,nano-subtle :height 130)))

     (mu4e-headers-button
      (or sender-name sender-address "Unknown")
      (cond  ((and child unread) 'nano-default)
             (child              'nano-light-faded)
             (unread             'nano-light-strong)
             (t                  'nano-light-strong))
     ...
     (cond (unread ;;(propertize " ● " 'face 'nano-default))
           ...
           (inbox
            (propertize " • " 'face 'nano-light-faded))
     ...
     (if tags (concat
               (mapconcat
                (function (lambda (tag)
                            (mu4e-headers-button
                             (format "[%s]" tag)
                             '(:inherit (nano-light-salient nano-light-strong))
...
(defun mu4e-headers-attach (msg)
  (cond ((memq 'flagged  (mu4e-message-field msg :flags))
         (propertize "!" 'face 'nano-light-strong))
...
(defface mu4e-headers-svg-tag-salient-face
  `((t :foreground ,nano-light-background
       ...
       :box (:line-width 1 :color ,nano-light-salient :style nil)
       :family ,(face-attribute 'default :family)
       :weight ,(face-attribute 'default :weight)
       :height ,(if (display-graphic-p)
                    (- (face-attribute 'default :height) 20)
...
(defface mu4e-headers-svg-tag-critical-face
  `((t :foreground ,nano-light-background
       ...
       :box (:line-width 1 :color ,nano-light-critical :style nil)
       :family ,(face-attribute 'default :family)
       :weight ,(face-attribute 'default :weight)
       :height ,(if (display-graphic-p)
                    (- (face-attribute 'default :height) 20)
